...
class FanyaCrawler:
            ...
            while True:
                ...
                for li in li_tags:
                    try:
                        ...
                        # 获取作业名称和状态
                        name_p = li.find("p")
                        assignment_name = name_p.get_text(strip=True) if name_p else "未知作业"
                        
                        status_p = name_p.find_next("p") if name_p else None
                        assignment_status = status_p.get_text(strip=True) if status_p else "未知状态"
    ...
    def _parse_questions(self, question_block: Tag) -> List[Question]:
        ...
        try:
            ...
            # 获取所有题目详情
            question_details = question_block.find_all("div", attrs={"aria-label": "题目详情"})
            questions = []
    ...
    def _parse_single_question(self, detail: Tag, answer_type: int) -> Optional[Question]:
        ...
        try:
            # 获取题目标题
            title_tag = detail.find("h3", attrs={"class": "mark_name colorDeep"})
            if not title_tag:
                return None
            ...
            if answer_type in [self.ANSWER_TYPES["单选题"], self.ANSWER_TYPES["多选题"]]:
                # 选择题
                answers_tag = detail.find("ul", attrs={"class": "mark_letter colorDeep qtDetail"})
                question_answers = self._normalize_answers(answers_tag.get_text()) if answers_tag else []
                
                answer_div = detail.find("div", attrs={"class": "mark_answer"})
                correct_tag = answer_div.find("span", attrs={"class": "rightAnswerContent workTextWrap"}) if answer_div else None
                correct_answer = correct_tag.get_text(strip=True) if correct_tag else ""
                ...
                # 填空题
                fill_tag = detail.find("dl", attrs={"class": "mark_fill colorGreen"})
                if fill_tag:
                    dd_tags = fill_tag.find_all("dd")
                    correct_answers = [dd.get_text(strip=True) for dd in dd_tags]
                else:
                    correct_answers = []
                ...
                # 判断题
                answer_div = detail.find("div", attrs={"class": "mark_answer"})
                correct_tag = answer_div.find("span", attrs={"class": "rightAnswerContent"}) if answer_div else None
                correct_answer = correct_tag.get_text(strip=True) if correct_tag else ""
    ...
    def get_assignment_questions(self, assignment: Assignment) -> List[Question]:
        ...
        try:
            ...
            soup = BeautifulSoup(response.text, "lxml")
            question_blocks = soup.find_all("div", attrs={"class": "mark_item"})
            ...
            logger.info(f"作业 {assignment.assignment_name} 获取到 {len(all_questions)} 道题目")
                    ...
                    for question in assignment.questions:
                        ...
                        if with_answers:
                            if isinstance(question.correct_answer, list):
                                f.write("正确答案: " + ", ".join(question.correct_answer) + "\n\n")
                            else:
        ...
        try:
            ...
            for assignment in assignments:
                ...
                for i, question in enumerate(assignment.questions, 1):
                    ...
                    if question.question_answers:
                        for answer in question.question_answers:
                            p = doc.add_paragraph(answer.strip(), style="List Bullet")
                    ...
                    if with_answers:
                        ...
                        if isinstance(question.correct_answer, list):
                            ans_text = "正确答案: " + ", ".join(question.correct_answer)
                        else:
...
def main():
    ...
    try:
        ...
        if args.format in ["markdown", "all"]:
            exporter.export_markdown(assignments, with_answers=not args.no_answers)
